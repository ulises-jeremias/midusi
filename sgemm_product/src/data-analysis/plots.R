...
ggplot(data = segmmProductData, aes(x = Run)) + geom_histogram(binwidth = binwidth,
  aes(fill = ..count..), alpha = 0.9) +
  labs(x = "Run", y = "cantidad")

ggplot(data = segmmProductData, aes(x = Run)) + geom_histogram(binwidth = binwidth,
  aes(y = ..density.., fill = ..count..), alpha = 0.9) +
  geom_density(col = 4) +
  labs(x = "Run", y = "densidad")
