...
ggplot(data = segmmProductData, aes(x = Run)) + geom_histogram(binwidth = binwidth, 
  aes(fill = ..count..), alpha = 0.9) + labs(x = "Run", y = "Cantidad")
...
ggplot(data = segmmProductData, aes(x = as.factor(NDIMA), fill = as.factor(NDIMA))) + 
  geom_bar() + scale_fill_hue(c = 40) + labs(x = "NDIMA", y = "Frecuencia", fill = "NDIMA")
